...
class ComputePowerSpectrum:
    ...
    def compute_power_spectrum_unbinned(self, grid):
        # Do the FFT transform with forward normalization and multiply by N to match FFTW convention
        fft_data = fftn(grid, norm='forward') * self.N
            ...
            for j in range(self.nbox):
                for k in range(self.nbox):
                    ...
                    if r != 0:
                        ...
                        g = self.w[i] ** 2 + self.w[j] ** 2 + self.w[k] ** 2
                        if g != 0: 
                            r = np.sqrt(g * self.coeff)
                            powspec[r] = powspec.get(r, 0.0) + contrib

        deltasqk = np.zeros(len(powspec), dtype=np.float64)
        ks = np.zeros(len(powspec), dtype=np.float64)
        
        for i, k in enumerate(sorted(powspec.keys())):
            deltasqk[i] = powspec.get(k, 0.0)
            ks[i] = k
        
        return (deltasqk, ks)

    def compute_power_spectrum_cyl(nbox, grid, box_size):
        # Total number of points in the grid (cubic)
        N = nbox * nbox * nbox 
        ...
        # Calculate power spectrum
        powspec_perp = np.zeros(nbox, dtype=np.float64)
        powspec_par = np.zeros(nbox, dtype=np.float64)
        for i in range(nbox):
            for j in range(nbox):
                for k in range(nbox):
                    # consider k as a 3D vector and calculate its magnitude squared
                    ...
                    if g_perp != 0 and g_par != 0:
                        # Calculate the index of the 'k' bin in the power spectrum
                        i_perp = int(0.5 + np.sqrt(g_perp * coeff))
                        i_par = int(0.5 + np.sqrt(g_par * coeff))
                        # Calculate the contribution to the power spectrum
                        contrib = (np.abs(fft_data[i,j,k]) ** 2)
                        #if (i==1 and j==1):
                        #    print(f"Contrib: {i} {j} {k} {g} {coeff} {i1} {fft_data[i,j,k]} {contrib}")
                        powspec_perp[i_perp] += contrib
                        powspec_par[i_par] += contrib
        ...
        for i in range(nhalf):
            powspec_perp[i] = powspec_perp[i] * (box_size ** 3.0) / (nbox ** 6.0) 
            powspec_perp[i] /= float(iweights[i])
            deltasqk[i] = (w[i] ** 3.0) * powspec_perp[i] / (2.0 * np.pi * np.pi)
